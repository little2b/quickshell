...
Singleton {
    ...
    property var _pinned: [
        {
            kind: "app",
            desktopId: "org.clavis.Launchpad"
        },
        {
            kind: "app",
    ...
    function activate(key, outputName) {
        ...
        if (entry && entry.desktopId === ApplicationService.launchpadApplication.id)
            return LaunchpadService.toggle(outputName);
    ...
    function launch(key) {
        ...
        if (!SpotlightAppUsage.launch(desktopId))
            return false;
        // Shell tools open synchronously without regular application windows.
        if (desktopId !== ApplicationService.settingsApplication.id && desktopId
                !== ApplicationService.launchpadApplication.id) {
