...
PanelWindow {
    ...
    readonly property bool pointerOverDock: shown && (bandHover.hovered || surfaceHover.hovered
                                                      && surfaceHover.point.position.x >= band.x
                                                      && surfaceHover.point.position.x < band.x + band.width
                                                      && surfaceHover.point.position.y >= band.y
                                                      && surfaceHover.point.position.y < band.y + band.height)
    onPointerOverDockChanged: {
        if (pointerOverDock) {
            const point = bandHover.hovered ? bandHover.point : surfaceHover.point;
            pointerAxis = horizontal ? point.scenePosition.x : point.scenePosition.y;
    ...
    Item {
        ...
        Item {
            ...
            width: root.horizontal ? Math.max(96, Math.min(root.availableLength, root.baseLayout.baseLength)) :
                                     root.shown ? root.edgeOffset + 4 : 3
            height: root.horizontal ? root.shown ? root.edgeOffset + 4 : 3 : Math.max(96, Math.min(
                                                                                          root.availableLength,
